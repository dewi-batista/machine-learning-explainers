...
\subsubsection{Learning $\mathcal{G}$ and $\theta$ from data}
...
\begin{tcolorbox}[title={\centering\textbf{Why aren't Bayesian networks employed in deep learning?}}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
...
In motivating variational autoencoders (VAEs\footnote{Original paper by Kingma: \url{https://arxiv.org/pdf/1312.6114}}), it's natural to first motivate regular autoencoders, illustrate issues pertaining to intuitive ideas of doing generative things with them and fixing those issues with variational inference.
...
% \end{align*}
% where $\z_1,\dots,\z_k\sim p(\z)$. Note that this is due to the choice of a prior $p_{\theta}(\z)$ which is independent of $\theta$, e.g. an isotropic Gaussian.
...
%     \nabla_{\phi}\text{ELBO}
%     &=
%     \nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x,\Z))-\log(q_{\phi}(\Z|\x))\r]\\
%     &\neq
%     \E_{\Z\sim q_{\phi}(\z|\x)}\l[\nabla_{\phi}(\log(p_{\theta}(\x,\Z))-\log(q_{\phi}(\Z|\x)))\r].
% \end{align*}
% So how might we get an unbiased estimate of $\nabla_{\phi}\text{ELBO}$? One might hope that we could switch up the distribution with which the expectation is taken. If the stochasticity of sampling latents in constructing a Monte Carlo estimate was due to something independent of $\phi$ then all good. To make this happen, if one can instead sample latents according to independent random noise $\epsilon\sim\mathcal{N}(0, I)$ through $\Z=g(\epsilon, \phi, \x)$ then we have
% \begin{align*}
%     \nabla_{\phi}\text{ELBO}
%     &=
%     \nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x,\Z))-\log(q_{\phi}(\Z|\x))\r]\\
%     &=
%     \nabla_{\phi}\E_{p(\epsilon)}\l[\log(p_{\theta}(\x,\Z))-\log(q_{\phi}(\Z|\x))\r]\\
%     &=
%     \E_{p(\epsilon)}\l[\nabla_{\phi}(\log(p_{\theta}(\x,\Z))-\log(q_{\phi}(\Z|\x)))\r]\\
%     &\approx
%     \frac{1}{k}\sum_{i=1}^k\nabla_{\phi}(\log(p_{\theta}(\x,\z_i))-\log(q_{\phi}(\z_i|\x)))
% \end{align*}
% where $\z_i=g(\epsilon_i, \phi,\x)$ and $\epsilon_1,\dots,\epsilon_k\sim p(\epsilon)$ (ignore the abuse of notation).
