% 1. Write unsupervised learning section:
%  PCA
%  k-means clustering
%  Isomap: https://en.wikipedia.org/wiki/Isomap
%  t-SNE
%  Autoencoders (already written)
%  Evaluating Unsupervised Learning Models

% 2. Include Hopfield networks and Boltzmann machines in generative models section
...
% abstract
\begin{abstract}
    \noindent Summaries of machine/deep learning-related topics. My main motive in writing these summaries is as a reminder for my future self.
\end{abstract}
...
I think of machine learning as the broad discipline of studying methods of fitting models to data — like statistics, as a discipline, with less rigour but more creative ideas. For a description of machine learning which helps to clarify why it's difficult to define as a term, consider the following excerpt from Herbert Jaeger's lecture notes for his machine learning course at the University of Groningen during the academic year 2023/24:
\begin{quotation}
...
An intuitive approach to finding the `optimal' parameters of a linear regression model, which we denote by $\theta^{*}$, is to split $D$ into training and testing datasets $D_{\text{train}}$ and $D_{\text{test}}$ (each consisting of $n$ sample in our case) and minimising some pre-determined loss function of said parameters over $D_{\text{train}}$. Essentially, minimising something like
$$
\mathcal{L}(\theta)
=
...
\begin{tcolorbox}[title={\textbf{Funny misuse of linear regression: Momentous sprint at the 2156 Olympics?}}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
    Read this Quora post\footnote{\url{https://qr.ae/pslbEN}} based on a paper\footnote{\url{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3173856/}} published in Nature. The top comment of the post is worth reading too. Related xkcd comic\footnote{\url{https://xkcd.com/1007/}}.
\end{tcolorbox}
...
\subsubsection{Batch + Layer + RMS normalisation}
...
\subsubsection*{3) RMS norm}
RMS norm is a slight simplification of layer norm in which the root mean square is used to normalise activations rather the standard deviation, i.e. we do not subtract the mean. As a result, the normalised activations don't need to have mean $0$. Given activation values $a_1,\dots,a_d$, RMS norm normalises as in
\begin{align*}
    (\hat{a}_1,\dots,\hat{a}_d)
    &=
    \l(\frac{a_1}{\sqrt{\frac{1}{d}\sum_{j=1}^da_j^2+\epsilon}},\dots,\frac{a_d}{\sqrt{\frac{1}{d}\sum_{j=1}^da_j^2+\epsilon}}\r)
\end{align*}
...
\begin{figure}[t]
    ...
    \begin{minipage}{0.55\linewidth}
        \caption{Motivating regularisation. Two fits, both with 0 error on selected datapoints.}
...
\begin{align*}
    ...
    &=
    \argmax_{\theta\in\R^{m+1}}\Big[\log(p(D|\theta))+\log(p(\theta))\Big]\\
    &=
...
G_i^{(t)}
=
\sum_{\tau=1}^t\l(\nabla_{\theta_i}R_D\l(\theta_i^{(\tau)}\r)\r)^2
$$
and switching the optimisation step to
$$
\theta_i^{(t+1)}
=
\theta_i^{(t)}-\eta_i\nabla_{\theta_i}^{(t)}R_D\l(\theta_i^{(t)}\r)
$$
...
\subsubsection{\NEXT Warmup + decay}
\dots

\subsubsection{Gradient accumulation + clipping}
\dots
...
Suppose research groups A and B independently publish results on nearly identical single hidden layer MLPs for the same task. The only difference is the number of neurons in the hidden layer: 99 for group A and 100 for group B. Their training setup is otherwise identical: same data, same splits, same optimisation. Group A reports a slightly lower benchmark test error, and so a company independent of either group adopts the architecture of group A. How is this fundamentally different from a single group training both 99- and 100-neuron models and then choosing the one with the lower test loss?
...
Multi-layer perceptrons (MLPs) are fully-connected feed-forward networks consisting of an input layer (where data is input), hidden layers and an output layer. An MLP with three hidden layers is illustrated in \autoref{fig:MLP}. Each layer is made up of a number of neurons, each of which has a real-valued activation value.  For any neuron in a non-input layer, its activation value is the output of a non-linear activation function given, as input, the neuron's real-valued bias plus a weighted sum of the activation values of all neurons in its preceding layer. Their intended use is as arbitrary function approximators. The output of the MLP, i.e. the output of one's approximation of the relevant underlying function, is given by the activation values of the neurons in the output layer. Regarding their inspiration from the human brain, consider this footnote\footnote{\url{https://stats.stackexchange.com/a/159172}}.
...
\begin{itemize}
    ...
    \item[--] $w_{i,l}^{(j)}\in\mathbb{R}$ is the weight associated with the edge to neuron $i$ in layer $j\in[k+1]$ from neuron $l$ in layer $j-1$.
...
\texttt{embed}
\rightarrow
\texttt{encode position}
\rightarrow
\texttt{trf blocks}
\rightarrow
\texttt{output proj}
$$
as illustrated in \autoref{fig:transformer-architecture}. Each step is detailed in this subsection.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{./figures/neural_nets/transformer.pdf}
    \caption{A decoder-only language transformer consisting of $N$ transformer blocks.}
...
$$
\colorbox{purple}{The}\colorbox{green}{chef}\colorbox{brown}{cooked}\colorbox{red}{a}\colorbox{teal}{meal}\colorbox{blue}{for}\colorbox{yellow}{his}\colorbox{orange}{critic}\colorbox{pink}{.}
$$
or splitting up an image as in \autoref{fig:image-tokenisation}. To play around with one of OpenAI's text tokenisers, consider \url{platform.openai.com/tokenizer}.

The purpose of tokenisation is self-explanatory: we'd like an idea of dependence between different parts of the input as to aid our output prediction. As such, being able to break a sequence into chunks, as opposed to processing it in its entirety, is desirable.  Notationally, let $V$ denote the set of all tokens in our model's vocabulary and $T:\Sigma^*\to \{1,\dots,|V|\}^*$ the tokenisation function where $S^*$ is the set of all finite strings whose characters are in $S$.

\begin{figure}[t]
    \centering
    \includegraphics[height=7cm, keepaspectratio]{./figures/neural_nets/augusto.jpg}
    \hfill
    \includegraphics[height=7cm, keepaspectratio]{./figures/neural_nets/augusto-tokenised.png}
    \caption{The tokenisation (or patchification) of a sleeping Augusto.}
    \label{fig:image-tokenisation}
\end{figure}

For text data, the tokeniser $T$ is often learned according to byte pair encoding (BPE). Roughly put, start with a vocabulary $V$ consisting of the first 256 UTF-8 encoding characters. Then, until the vocabulary size reaches some threshold, say $50,000$, repeat the following for $i=0,\dots,V_{\text{max}}-1$:
\begin{enumerate}
    \item Tokenise the text according to the tokeniser
    \item Compute the most frequent token pair in the corpus and append it to the tokensier with index $256+i$.
\end{enumerate}
The result is an ordered list of merge rules. For example, the most frequent character pair in English text 

\subsubsection{Embedding}
A linear embedding map $L_E:\{e_1,\dots,e_{|V|}\}\to\mathbb{R}^d$, where $d\in\mathbb{N}$ is the embedding dimension and $e_i$ denotes the $i^{\text{th}}$ standard basis vector, offers high dimensional numeric representations of tokens belonging to the vocabulary $V$. What makes token embedding functions interesting is how well they capture underlying semantics, e.g. of natural language. Such an embedding map learned on pieces of text, like Word2Vec, might satisfy
$$
L_E(\text{``Germany"})+L_E(\text{``fascist"})-L_E(\text{``Mussolini"})
\approx
L_E(\text{``Hitler"})
$$
or
$$
L_E(\text{``Wales"})+L_E(\text{``Cardiff"})-L_E(\text{``Italy"})
\approx
L_E(\text{``Rome"})
$$
where ``word'' denotes the basis vector corresponding to the word according to some indexing over the vocabulary, e.g. perhaps $|V|=50,000$ and ``Germany''$=e_{35,629}$. The prupose of taking the domain of $L_E$ to be this set of standard basis vectors is that then it can be seen as a look-up table of the rows of $E\in\R^{d\times|V|}$.

So how is the embedding matrix $E\in\R^{d\times |V|}$ learned? In the context of embedding maps like Word2Vec, where the embedding map is learned in isolation (not in conjunction with a larger architecture), it is learned by repeatedly sampling a center word from the dataset (a large corpus of text in our example), a context word which lies near the center word in the text and $l$ negative-context words which do not lie near the center word. The loss function used to learn the parameters (the elements of our linear embedding map) intends to reward the alignment of the context word and the center word while punishing the alignment of the center word and the $l$ negative-context words. In maths notation, the loss looks something like
$$
-\sigma(w_{\text{cent}}, w_{\text{cont}}) + \sum_{i=1}^l\sigma\l(w_{\text{cent}}, w_{\text{neg}}^{(i)}\r)
$$
where $\sigma$ denotes the sigmoid function.

Embedding maps are less interesting when learned in conjunction with a larger architecture, e.g. GPT architectures, as they are learned according to backpropagation like any other parameter. If the loss is not such that alignments of token embeddings in $\R^d$ mean anything, some of the nice properties discussed earlier are less likely to be observed.

\subsubsection{Positional encoding/embedding}
...
$$
\colorbox{purple}{The}\colorbox{orange}{critic}\colorbox{brown}{cooked}\colorbox{red}{a}\colorbox{green}{chef}\colorbox{blue}{for}\colorbox{yellow}{his}\colorbox{teal}{meal}\colorbox{pink}{.}
$$
In the latter sequence, the intended meaning of \colorbox{brown}{cooked} is akin to `criticised', while in the former it is simply meant to convey that the chef literally cooked. So how do transformers go about accounting for the positions of tokens in given input sequences?
...
Let $\x=(x_1,\dots,x_n)\in\{e_1,\dots,e_{|V|}\}^n$ denote a sequence of $n$ one-hot encoded tokens (so the post-tokenisation input) and
$$
E(\x)
=
\begin{bmatrix}
...
\in\R^{n\times d}
$$
their embedding. Given an appropriate positional embedding matrix $P(\x)\in\R^{n\times d}$ many set-ups form the input to the first transformer block by computing its sum with the embedding matrix $E(\x)$. That is, they compute
$$
X
=
E(\x)+P(\x)\in\R^{n\times d}
$$
...
\begin{itemize}
    \item \textbf{Why not simply add the index of the token to its embedding?} For long sequences, the index value of later tokens would overpower the value of the presence of the token itself. In line with this, we'd like the positional encoding to be bounded in magnitude.
    \item \textbf{Why not add the index divided by the sequence length to ensure getting something in [0,1]?} For sequences of varying length, we'd like the positional information for a given index to be the same. So attributing 0.5 to the third token in a sequence of five would be an issue as it would also be attributed to the 11th token in a sequence of 20.
    \item \textbf{Why not encode via the binary of the index?} Change in positional encoding should be smooth. We want $||P_{k+1,:}-P_{k,:}||$ to be bounded but for binary encodings it is simply equal to the square root of the number of bit flips: unbounded in $k$.
\end{itemize}
To address both issues, the authors of the now famous Attention Is All you Need proposed alternating sinusoids\footnote{Great video about it: \url{www.youtube.com/watch?v=dWkm4nFikgM}.}. Sinusoidal positional encoding feels unnatural to me, though it has a nice motivation as a continuous analog of binary encoding. Regardless, far more intuitive methods of positional encoding exist such as learned positional encoding (used by OpenAI for GPT-2) or the now-popular rotary positional embeddings (RoPE).

Learned positional encoding is just as the name suggests: the positional embedding map $P\in\R^{n\times d}$ is set up as a matrix of parameters which are learned according via gradient descent in conjunction with all other model parameters. RoPE is bit more involved than learned positional encoding as it's applied during the attention mechanism and not between embedding and feeding the input to the first transformer block. Despite this complication, RoPE is quite elegant, relying on the linearity of rotation transformations in $\R^d$. I'll detail them here at some point but, for now, knowing just that positional embedding is a core component of transformer architectures is sufficient.

\subsubsection{\NEXT Attention + MLP component}
Denote the input to a transformer block by $X$, e.g. in the learned positional encoding scenario this is post-tokenisation, post-embedding and post-positional encoding. Each transformer block first applies its attention mechanism to $X$\footnote{Strictly speaking, the first transformation applied to the input might be some form of normalisation, e.g. layer normalisation. This is ignored for the sake of simplicity.}.

The intended purpose of attention is to obtain some representation of how much past tokens contribute to the intended meaning of the current token. For example, \dots. Using linear transformations, \dots
$$
A
=
\text{Softmax}\l(\frac{QK^T}{\sqrt{d}}+M\r)
$$
where the causal matrix $M\in\R^{?\times ?}$ is zeros except for above the diagonal which are filled with representations of $-\infty$ in the relevant framework (e.g. PyTorch).

Note that due to the computational expense of attention, a ton of effort is put into developing attention-like methods whose computational complexities are less harsh, e.g. $O(n\log(n))$ or $O(n)$. In fact, 2025 was quite the year for such developments. Since this bottleneck is what dictates the allowed context length, it's perhaps one of the more interesting parts of LLM architecture developments.

\subsubsection{Example: GPT-2 small ($\sim$123M parameters)}
GPT-2 small acts as a great introduction to how a decoder-only language transformer may look and the distribution of parameter counts over different components in their architecture. Its architecture is illustrated in \autoref{fig:transformer-architecture} and its hyperparameters are given in \autoref{tab:GPT-2-small-hyperparams}
\begin{table}[h]
    \centering
    \begin{tabular}{c|c}
        ...
        $d_{\text{ff}}$ & 3072
    \end{tabular}
    \caption{Hyperparameters for GPT-2-small.}
    \label{tab:GPT-2-small-hyperparams}
\end{table}
From the hyperparameters and the GPT-2-small's architecture, we can compute the total number of parameters of the model as in \autoref{tab:GPT-2-small-parameter-count}.
\begin{table}[h]
    \centering
    \begin{tabular}{c|c}
        \textbf{Parameter} & \textbf{Count (in millions)}\\
        \hline\hline
        $E$ & $768\cdot50{,}256\approx38.6$\\
        \hline
        $P$ & $1024\cdot768\approx0.8$\\
        \hline
        $W_Q$ & $12\cdot768\cdot768\approx7.1$\\
        \hline
        $W_K$ & $12\cdot768\cdot768\approx7.1$\\
        \hline
        $W_V$ & $12\cdot768\cdot768\approx7.1$\\
        \hline
        $W_O$ & $12\cdot768\cdot768\approx7.1$\\
        \hline
        $W_1$ & $12\cdot768\cdot3072\approx28.3$\\
        \hline
        $W_2$ & $12\cdot768\cdot3072\approx28.3$\\
        \hline
        \textbf{Total} & $\sim124.4$
    \end{tabular}
    \caption{Parameter count for GPT-2-small.}
    \label{tab:GPT-2-small-parameter-count}
\end{table}
...
\end{tikzpicture}
\end{center}
with $v_1=x_1$, $v_2=x_2$, $v_4=x_3$, $v_3=\theta_1v_1v_2$ and $v_5=\theta_2+v_3+v_4$. In this representation, each node is a primitive function of its parents and the parameters. As such, we have $\frac{\partial v_5}{\partial \theta_1}=0$ but $\frac{dv_5}{d\theta_1}=v_1v_2$. This distinction between partial and total derivatives is important for our formulation of reverse-mode auto-differentiation, i.e. backpropagation.

\subsubsection{Reverse-mode auto-differentiation (backprop)}
...
\nabla_{\theta}v_N
=
\l(\frac{d}{d\theta_1}v_N,\dots,\frac{d}{d\theta_m}v_N\r).
$$
...
\frac{d}{d\theta_i}v_N
=
\sum_{k=1}^N\frac{dv_N}{dv_k}\frac{\partial v_k}{\partial\theta_i}
$$
with
$$
\frac{d}{dv_k}v_N
=
\sum_{v_c\in\textbf{Ch}(v_k)}\frac{dv_N}{dv_c}\frac{\partial v_c}{\partial v_k}
$$
where $\textbf{Ch}(v)$ denotes the set of children of the node $v$ in the computational DAG. As a result, if $\partial v_c/\partial v_k$ and $\partial v_k/\partial\theta_i$ have closed forms for all $v_c\in\textbf{Ch}(v_k)$ and $k\in\{1,\dots,N\}$ then the desired total derivatives can be computed by simply computing $dv_N/dv_k$ for $k=N-1,\dots,1$ (in said reversed order) and plugging them in to the equation above. This makes it immediately clear why it is named reverse-mode auto-differentation or backpropagation.

As an explicit example, consider the DAG with $v_1,\dots,v_5$ above. After some algebra, we obtain
$$
\frac{dv_5}{dv_4}=\frac{\partial v_5}{\partial v_4} \hspace{20pt}
\frac{dv_5}{dv_3}=\frac{\partial v_5}{\partial v_3} \hspace{20pt}
\frac{dv_5}{dv_2}=\frac{\partial v_5}{\partial v_3}\frac{\partial v_3}{\partial v_2} \hspace{20pt}
\frac{dv_5}{dv_1}=\frac{\partial v_5}{\partial v_3}\frac{\partial v_3}{\partial v_1}
$$
and the only non-zero partial derivatives with respect to parameters are
$$
\frac{\partial v_3}{\partial\theta_1}=v_1v_2
\hspace{10pt}\text{and}\hspace{10pt}
\frac{\partial v_5}{\partial\theta_2}=1.
$$
From these, we obtain closed forms of the desired total derivatives
\begin{align*}
    \frac{dv_5}{d\theta_i}
    =
    \sum_{k=1}^5\frac{dv_5}{dv_k}\frac{\partial v_k}{\partial\theta_i}
    =
    \frac{\partial v_3}{\partial\theta_i}+\frac{\partial v_5}{\partial\theta_i}
\end{align*}
which yields $dv_5/d\theta_1=v_1v_2$ and $dv_5/d\theta_2=1$. Noting that
$$
v_5
=
\theta_2+v_4+\theta_1v_1v_2,
$$
we see that these total derivatives are correct. This example helps to illustrate the need for closed forms of the partial derivatives $\partial v_k/\partial\theta_i$ and $\partial v_c/\partial v_k$ for all $v_c\in\textbf{Ch}(v_k)$ and $k\in\{1,\dots,N\}$. Due to this, backpropagation relies on each node being a primitive function of its inputs and parameters.
...
\begin{itemize}[label=--, itemsep=0pt, topsep=2pt]
    ...
    \item Normalisations
    \item Reduction operations, e.g. sum, mean, etc.
    \item Dropout
    \item Losses
    \item Convolutions/pooling
    \item Indexing and reshaping
    \item Attention-related operations
\end{itemize}
Computing closed forms of the relevant partial derivatives can be fun. There are simple ones, e.g. if $v_2=\sigma(v_1)$ where $\sigma$ is the sigmoid activation function then $\partial v_2/\partial v_1=\sigma'(v_1)$ has a simple closed form. For an example of the parameters, consider a simple linear layer as in $v_2=\theta_1v_1+\theta_2$ where $\theta_1$ and $\theta_2$ are scalar parameters. In this case, $\partial v_2/\partial\theta_1=v_1$. Simple enough.

\begin{tcolorbox}[title={\centering\textbf{Why not forward-mode auto-differentiation?}}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
    Forward-mode auto-differentiation is a similar idea with the relevant derivatives computed from the input nodes to the output nodes. For $f:\R^n\to\R^m$, computing the full Jacobian using forward-mode auto-differentiation requires $O(n)$ memory and $O(n\#\text{DAG})$ time. For comparison, backpropagation requires $O(\#\text{DAG})$ memory and $O(m\#\text{DAG})$ time. It's immediately clear that in machine learning contexts, where $n$ far exceeds $m=1$, backpropagation is massively favourable if you have sufficient memory.

    \hspace{14.5pt}Other methods exist, e.g. numerical differentiation and symbolic differentiation. The former suffers for many reasons, like instability due to high-precision requirements, and the latter suffers due to the massive blow up of function representations.
\end{tcolorbox}

\subsubsection{Backpropagation for MLPs}
As with formulating MLPs, it's good to rigorously derive backpropagation for them at least once\footnote{I wrote the following before writing about reverse-mode auto-differentiation above and so the notation here is in line with the formulation of MLPs.}. It's sort of strange that I haven't done that yet (August 2025 is time of writing) and have just trusted that it holds up.
...
% (deep) generative models (all other section names i've come up with felt overthought)
\section{Deep Generative Modelling}
Broadly speaking, a generative model seeks to offer ways to reason about the data-generative process to which it corresponds. Reasoning here could mean simulating the data-generative process (sampling) or answering questions pertaining to the frequency of certain events. The simplest example of a generative model I can think of with respect to this broad definition is a histogram: a distribution of the frequencies of events, as in \autoref{fig:histogram}. A histogram allows one to see which event is most frequent, the portion of events which satistfy some condition (e.g. exceeding some quantitative threshold) or to simulate the data-generative process. So how do we go from generative models to generative probabilistic models? Well, where frequency-based reasoning goes, probabilistic reasoning follows!

\begin{figure}[t]
    \centering
    \begin{minipage}{0.64\linewidth}
        \includegraphics[width=\textwidth]{./figures/generative_models/histogram.pdf}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.36\linewidth}
        \caption{A histogram pertaining to the frequency of states of weather. Corresponding probabilities in parentheses.}
        \label{fig:histogram}
    \end{minipage}
\end{figure}

A generative probabilistic model is a tuple $(\X, p_{\Theta})$ consisting of model variables $\X=(X_1,\dots,X_m)$ and a probability function $p_{\Theta}:\Omega_{\X}\rightarrow\mathbb{R}_{\geq0}$ parameterised according to the parameter set $\Theta$. Given a generative probabilistic model, we might hope to answer evidence queries $p_{\Theta}(\x)$, marginal queries $p_{\Theta}(\x')$, where $\x'$ is a realisation of some subset of the model variables, and sampling queries $\hat{\x}\sim p_{\Theta}(\x)$, in which one simulates the data-generative process in turn producing samples of the underlying distribution.

The holy grail of generative probabilistic modelling is to find an exact, efficiently computable and algebraically manipulable representation $p_\Theta$ of the underlying joint probability function of interest. If obtained, one can reason probabilistically with ease but, as we will see, the purpose, limitations and learning of generative probabilistic models from data varies heavily. To illustrate this, consider probabilistic graphical models (PGMs) which offer human-interpretable representations of conditional (in)dependencies between model variables according to a directed acyclic graph (DAG). While PGMs are not employed in deep learning contexts, they act as an important stepping stone in understanding contemporary methods. The most well-known class of PGMs are Bayesian networks.

% bayesian nets
\subsection{Bayesian networks}
A natural approach to representing the joint probability function of model variables is to decompose it into a product of probability functions over subsets of the model variables. For example, you might decompose the probability function according to the chain rule of probability as in
\begin{align*}
    p(x_1,\dots,x_m)
    &=
...
\end{align*}
From here, the probabilistic query of sampling, for example, may be done autoregressively: sample $x_1$ according to $p(x_1)$, $x_2$ according to $p(x_2|x_1)$ and so on until an entire realisation of $\X=(X_1,\dots,X_m)$ is obtained. This idea leads to the natural motivation of Bayesian networks: why stop at decomposition according to the chain rule? In image date, for example, nearby pixel values may correlate but far away pixels may not, so why incoroporate distant pixels in a given conditional in the learned decomposition? Why not learn the conditional (in)dependencies of the model variables of interest and decompose accordingly? That's exactly what Bayesian networks do!

With their motivation out of the way, we can move on to their formulation. The parameter set $\Theta=(\mathcal{G},\theta)$ of a Bayesian network $(\X,p_\Theta)$ consists of a directed acyclic graph (DAG) $\mathcal{G}$ and a parameter set $\theta$. The graph $\mathcal{G}$ has a single node for each model variable $X_1,\dots,X_m$ and directed edges represent conditional (in)dependencies. The parameter set $\theta$ pertains to the parameters of the conditional distributions (the smaller distributions in the decomposition) made explicit by $\mathcal{G}$. The representation of the joint probability function is given by
$$
...
\begin{example}
    For model variables $\X=(X_1,X_2,X_3)$, if $X_3|X_1$ is independent of $X_2$ then their joint probability function decomposes according to
    $$
...
\subsubsection{Learning them from data}
I won't go into particular detail regarding how one learns a Bayesian network (its graph $\mathcal{G}$ and parameter set $\theta$) from data as the nitty gritty details aren't relevant to the generative models relevant to deep learning. That said, there are some interesting takeways, especially from methods of learning a Bayesian network's structure $\mathcal{G}$ as they contrast the almost entirely-heuristic approaches taken in deep learning contexts.

Unlike with neural network architectures, both the parameter set $\theta$ and the structure $\mathcal{G}$ of a Bayesian network can be motivated in a principled manner via maximum likelihood estimation. In brief, the likelihood of a given parameter set and DAG can be computed separately, and so in learning a Bayesian network from a dataset $D=\{\x_1,\dots,\x_n\}\overset{\text{i.i.d.}}{\sim}p(\x)^{\otimes n}$ one might hope to compute
\begin{align*}
    \argmax_{\mathcal{G}}\l[\text{Score}(\mathcal{G})\r]
    =&\argmax_{\mathcal{G}}\l[\argmax_{\theta}\l[\log\l(p_{(\mathcal{G}, \theta)}(D)\r)\r]-\text{penalty}(\mathcal{G})\r]\\
    =&\argmax_{\mathcal{G}}\l[\argmax_{\theta}\l[\sum_{i=1}^n\log\l(p_{(\mathcal{G}, \theta)}(\x_i)\r)\r]-\text{penalty}(\mathcal{G})\r]
\end{align*}
...
\begin{enumerate}
    \item Compute the scores of all graphs which add or remove an edge from the current graph (or reverse an edge).
    \item Edit the graph according to whichever addition, removal or reversal furthest increases the score.
    \item No change in score? Stop, return $\mathcal{G}$ and recover $\theta$ accordingly.
\end{enumerate}
...
\subsubsection{Why aren't they employed in deep learning?}
Learning their structure and parameters for high-dimensional distributions becomes quickly computationally infeasible (say from $\sim100$). Regardless of this `weakness', they serve as a significant, and often overlooked, step in the evolution of probabilistic reasoning and, more broadly, generative probabilistic modelling.
...
Note that VAEs can be motivated in a ton of other ways, e.g. Kingma (their creator) himself describes them in terms of Bayesian networks and latent variable models. Such a motivation can be found in \url{https://arxiv.org/pdf/1312.6114} which acts as a great source for learning about VAEs.
...
\begin{align*}
    \log(p_{\theta}(\x))
    &=
    \log\l(\int p_{\theta}(\x|\z)p_{\theta}(\z)d\z\r)\\
    &=
    \log\l(\int q_{\phi}(\z|\x)\frac{p_{\theta}(\x|\z)p_{\theta}(\z)}{q_{\phi}(\z|\x)}d\z\r)\\
    &=
    \log\l(\E_{\Z\sim q_{\phi}(\z|\x)}\l[\frac{p_{\theta}(\x|\Z)p_{\theta}(\Z)}{q_{\phi}(\Z|\x)}\r]\r)\\
    &\geq
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log\l(\frac{p_{\theta}(\x|\Z)p_{\theta}(\Z)}{q_{\phi}(\Z|\x)}\r)\r]\\
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x|\Z))\r]-\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log\l(\frac{q_{\phi}(\Z|\x)}{p_{\theta}(\Z)}\r)\r]\\
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x|\Z))\r]-D_{\text{KL}}(q_{\phi}(\Z|\x)||p_{\theta}(\Z))\\
    &=:
    \text{ELBO}
\end{align*}
in which the inequality arises due to Jensen's inequality, as in $\E[\log(f(\Z))]\leq\log(\E[f(\Z)])$, and $D_\text{KL}(Q||P)$ denotes the KL-divergence between distributions $Q$ and $P$, which is detailed in the appendix. It's worth noting that equivalent representations of the ELBO exist and are useful in motivating VAEs, as in
\begin{align*}
    \text{ELBO}
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x|\Z))\r]-\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log\l(\frac{q_{\phi}(\Z|\x)}{p_{\theta}(\Z)}\r)\r]\\
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log\l(\frac{p_{\theta}(\x,\Z)}{p_{\theta}(\Z)}\r)-\log\l(\frac{q_{\phi}(\Z|\x)}{p_{\theta}(\Z)}\r)\r]\\
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x,\Z))-\log(q_{\phi}(\Z|\x))\r]
\end{align*}
or
\begin{align*}
    \text{ELBO}
    ...
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x,\Z))-\log(q_{\phi}(\Z|\x))\r]\\
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\Z|\x)p_{\theta}(\x))-\log(q_{\phi}(\Z|\x))\r]\\
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x))-\log\l(\frac{q_{\phi}(\Z|\x)}{p_{\theta}(\Z|\x)}\r)\r]\\
    &=
    p_{\theta}(\x)-D_{\text{KL}}\l(q_{\phi}(\Z|\x)||p_{\theta}(\Z|\x)\r).
    \label{eq:vae-elbo-3}
\end{align*}
The effect of maximising the ELBO can be motivated in a number of ways. Its statement directly above makes it immediately clear that its maximisation corresponds to maximum likelihood and the minimisation of the KL-divergence between the model posterior $q_{\phi}(\Z|\x)$ and the induced posterior $p_{\theta}(\Z|\x)$. That said, $p_{\theta}(\Z|\x)$ is intractable (which is why we learn the model posterior $q_{\phi}(\Z|\x)$), and so I prefer to motivate the maximisation of the ELBO term-by-term with respect to the following form
$$
\text{ELBO}
=
\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x|\Z))\r]-D_{\text{KL}}(q_{\phi}(\Z|\x)||p_{\theta}(\Z)).
...
\l[
\sum_{i=1}^n
\E_{\Z\sim q_{\phi}(\z|\x_i)}\l[\log(p_{\theta}(\x_i|\Z))\r]-D_{\text{KL}}(q_{\phi}(\Z|\x_i)||p_{\theta}(\Z))
\r].
$$
...
\subsubsection{Backpropagation for VAEs (the reparameterisation trick)}
To find suitable parameters using gradient descent, we require estimates of $\nabla_{\theta}\text{ELBO}$ and $\nabla_{\phi}\text{ELBO}$. The former is straightforwardly estimated using Monte Carlo estimation as in
\begin{align*}
    \nabla_{\theta}\text{ELBO}
    &=
    \nabla_{\theta}\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x|\Z))\r]\\
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\nabla_{\theta}\log(p_{\theta}(\x|\Z))\r]\\
    &\approx
    \frac{1}{k}\sum_{j=1}^k \nabla_{\theta}\log(p_{\theta}(\x|\z_j)).
\end{align*}
where $\z_1,\dots,\z_k\sim q_{\phi}(\z|\x)$. A closed form is often obtained by taking convenient distributions, e.g. $p_{\theta}(\x|\z)=\mathcal{N}(\x;\mu_{\theta}(\z),\sigma_{\theta}(\z)I)$.

Estimating $\nabla_{\phi}\text{ELBO}$, however, is tricky. The expectation itself is with respect to $q_{\phi}(\z|\x)$ which explicitly depends on $\phi$ and so the gradient $\nabla_{\phi}$ can not be brought inside. That is,
$$
\nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}[f(\Z)]
\neq
\E_{\Z\sim q_{\phi}(\z|\x)}[\nabla_{\phi}f(\Z)].
$$
Before we proceed by describing the reparameterisation trick, it's worth noting that tehre is a closed form for $\nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}[f(\Z)]$ referred to as the score function estimator given by
$$
\nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}[f(\Z)]
=
\E_{q_{\phi}(\z|\x)}[f(\Z)\nabla_{\phi}\log\l(q_{\phi}(\Z|\x)\r)]
$$
...
Suppose instead that you MC-estimated $\nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}[f(\Z)]$. The issue here is that you would then have to compute the gradient through the process that yielded the samples $\z_1,\dots,\z_k$ contributing to your estimate. For example, if
$$
\z_i
=
F_{\phi}^{-1}(u_i)
$$
where $u_1,\dots,u_k\sim\text{Unif}(u;0,1)$ then the process by which our latents are obtained is not deterministic but explicitly depends on $\phi$. How then does one backpropagate through this process? Auto-differentiation isn't a fan. The idea of the reparameterisation trick is very natural: entirely separate the stochasticity obtaining said samples from the parameters $\phi$. This can be done by taking $\Z|\x=g(\bm{\epsilon}; \phi, \x)$ for invertible $g$ in which $\bm{\epsilon}\sim\mathcal{N}(0, I_d)$ is the only stochastic component of sampling. With such a function $\Z=g(\bm{\epsilon};\phi,\x)$ we have (via the law of the unconscious statistician)
\begin{align*}
    \nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}[f(\Z)]
    &=
    \nabla_{\phi}\E_{p(\bm{\epsilon})}[f(\Z)]\\
    &=
    \E_{p(\bm{\epsilon})}[\nabla_{\phi}f(\Z)]
\end{align*}
...
\begin{align*}
    \nabla_{\phi}\text{ELBO}
    &=
    \nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x|\Z))+\log(p_{\theta}(\Z))-\log(q_{\phi}(\Z|\x))\r]\\
    &=
    \nabla_{\phi}\E_{p(\bm{\epsilon})}\l[\log(p_{\theta}(\x|\Z))+\log(p_{\theta}(\Z))-\log(q_{\phi}(\Z|\x))\r]\\
    &=
    \E_{p(\bm{\epsilon})}\l[\nabla_{\phi}\l(\log(p_{\theta}(\x|\Z))+\log(p_{\theta}(\Z))-\log(q_{\phi}(\Z|\x))\r)\r]\\
    &\approx
    \frac{1}{k}\sum_{i=1}^k\nabla_{\phi}\l(\log(p_{\theta}(\x|\z_i))+\log(p_{\theta}(\z_i))-\log(q_{\phi}(\z_i|\x))\r)
\end{align*}
where $\z_i=g(\bm{\epsilon}_i;\phi,\x)$ and $\bm{\epsilon}_1,\dots,\bm{\epsilon}_k\sim\mathcal{N}(\bm{\epsilon};0,I_d)$. Ignore my abuse of notation of conflating the random variable $\bm{\epsilon}\sim\mathcal{N}(0,I_d)$ with corresponding samples. This estimator is unbiased and all that good stuff. For an illustration of how the reparameterisation helps alleviate the earlier issue with backpropagation, consider \autoref{fig:reparameterisation_trick}.

An explicit example of such a reparameterisation is when the posterior is a facorised Gaussian. That is, $q_{\phi}(\z|\x)=\mathcal{N}(\z;\mu_{\phi}(\x), \text{diag}(\sigma_{\phi}^2(\x)))$. In this case, take $g(\bm{\epsilon};\phi,\x)=\mu_{\phi}(\x)+\sigma_{\phi}(\x)\odot\bm{\epsilon}$ where $\bm{\epsilon}\sim\mathcal{N}(\bm{\epsilon};0,I_d)$ and $\odot$ denotes element-wise multiplication. Typically, the prior $p_{\theta}(\z)$ and the likelihood $p_{\theta}(\x|\z)$ are also taken such that their logarithm is nice to compute and backproppable.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.49\columnwidth,height=4cm,keepaspectratio]{./figures/generative_models/reparameterisation.pdf}
    \hspace{15pt}
    \includegraphics[width=0.49\columnwidth,height=4cm,keepaspectratio]{./figures/generative_models/reparameterisation-backprop.pdf}
    \caption{Backpropagation before and after applying the reparameterisation trick. Deterministic nods in blue and stochastic nodes in red.}
    \label{fig:reparameterisation_trick}
\end{figure}

\begin{tcolorbox}[title={\centering\textbf{Computing} $\log(q_{\phi}(\z|\x))$}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
    Though nice distributions are often attributed to the posterior, it's worth noting that there's a convenient manner of computing $\log(q_{\phi}(\z|\x))$ which relies on the invertibility of $\Z=g(\bm{\epsilon};\phi,\x)$. Note that
    \begin{align*}
        \log(q_{\phi}(\z|\x))
        &=
        \log\l(p(\bm{\epsilon})\l|\det\l(\frac{\partial \z}{\partial\bm{\epsilon}}\r)\r|^{-1}\r)\\
        &=
        \log(p(\bm{\epsilon}))-\log\l(\l|\det\l(
        \begin{bmatrix}
            \frac{\partial z_1}{\partial\epsilon_1} & \cdots & \frac{\partial z_1}{\partial\epsilon_d}\\
            \vdots & \ddots & \vdots\\
            \frac{\partial z_d}{\partial\epsilon_1} & \cdots & \frac{\partial z_d}{\partial\epsilon_d}
        \end{bmatrix}
        \r)\r|\r)
    \end{align*}
    where $\bm{\epsilon}=(\epsilon_1,\dots,\epsilon_d)$. This choice is particularly useful for complicated-looking choices of the posterior.
\end{tcolorbox}

\subsubsection{Blurry reconstructions}
...
$$
D_{\text{KL}}(q_{D,\phi}(\X,\Z)||p_{\theta}(\X,\Z))
=
\E_{(\X,\Z)\sim q_{D,\phi}(\x,\z)}\l[\log\l(\frac{q_{D,\phi}(\X,\Z)}{p_{\theta}(\X,\Z)}\r)\r]
$$
where $q_{D,\phi}(\x,\z)$ is induced by $q_{\phi}(\z|\x)$ and the empirical data distribution $q_D(\x)$. The KL-divergence clearly illustrates that
\begin{enumerate}
    \item If $(\x,\z)$ is unlikely according to $q_{D,\phi}$ then the KL-divergence (which corresponds to the loss) is relatively low regardless of how likely it is under $p_{\theta}$.
    \item If $(\x,\z)$ is likely according to $q_{D,\phi}$ but unlikely according to $p_{\theta}$ then the KL-divergence blows up massively.
\end{enumerate}
As such, the model is encouraged to ensure that $p_{\theta}(\x,\z)$ is never small near observed data. This boils down to ensuring that the reconstruction likelihood $p_{\theta}(\x|\z)$ is not small (since $q_{\phi}(\z|\x)$ is regularised toward $p_{\theta}(\z)$, sampled latents typically lie in reasonably high prior-density regions). With a unimodal likelihood (e.g. Gaussian with fixed variance), the safest fit when distinct $\x$ share similar $\z$ is to place the conditional mean between them which results in blur. Note also that, even though the model is not penalised for what $p_{\theta}(\x|\z)$ does for $(\x,\z)$ such that $q_{D,\phi}(\x,\z)\approx0$, $p_{\theta}$ is still subject to normalising to 1 under its support. Due to this, $p_{\theta}$ can't get too wild in interpolating between the data.

As illustration, suppose two samples $\x_1$ and $\x_2$ map to the same latent under the encoder. That is, suppose $\z_1\approx\z_2$ for $\z_1\sim q_{\phi}(\z|\x_1)$ and $\z_2\sim q_{\phi}(\z|\x_2)$. Denote both by $\z_0$. Note that this is entirely plausible as the maximisation of the ELBO encourages the encoder to resemble the simple prior, e.g. the standard multivariate Gaussian with low latent dimension, and so it may be the case that there simply isn't enough room in the latent space to assign unique latents to distinct samples. In such a case, if $p_{\theta}(\x|\z_0)$ is unimodal, e.g. a Gaussian (which is often the case for vanilla VAEs), then where should its mean lie? That is, which of the samples which are sent to it by the encoder should it favour for reconstruction? If $\X|\z\sim\mathcal{N}(\mu_{\theta}(\z), \sigma^2I)$ then maximising the reconstruction term in the ELBO requires us to compute
\begin{align*}
    \argmax_{\theta}\l[\sum_{i=1}^2\log(p_{\theta}(\x_i|\z_0))\r]
    &=
    \argmax_{\theta}\l[-\frac{1}{2\sigma^2}\sum_{i=1}^2||\x_i-\mu_{\theta}(\z_0)||^2\r]\\
    &=
    \argmin_{\theta}\l(||\x_1-\mu_{\theta}(\z_0)||^2+||\x_2-\mu_{\theta}(\z_0)||^2\r)
\end{align*}
which corresponds to $\mu_{\theta}(\z_0)=\argmin_{\mu}\sum_{i=1}^2||\x_i-\mu||^2=(\x_1+\x_2)/2$.

\subsubsection{Working in latent space}
\dots
...
This is precisely the idea that motivates GANs with inspiration taken from game theory. A GAN consists of a generative component $G:\Omega_{\Z}\to\Omega_{\X}$ and a discriminative component $D:\Omega_{\X}\to(0,1)$, which are trained via alternating gradient updates in the style of a minimax game. That is, $G$ produces a new sample given a latent $\z\in\Omega_{\Z}$ while $D$ learns to distinguish between training samples and outputs of $G$. Essentially, the models compete. In terms of loss functions, the discriminator employs binary cross entropy, as in
\begin{align*}    
    \mathcal{L}_D
    &=
    -\E_{\X\sim p}\l[\log(D_{\phi}(\X))\r]-\E_{\Z\sim\mathcal{N}(0,I)}\l[\log\l(1-D_{\phi}(G_{\theta}(\Z))\r)\r]\\
    &\approx
    -\frac{1}{k}\sum_{i=1}^k\Big[\log(D_{\phi}(\x_i))+\log(1-D_{\phi}(G_{\theta}(\z_i)))\Big]\\
    &=
    -\frac{1}{k}\sum_{i=1}^k\log(D_{\phi}(\x_i)(1-D_{\phi}(G_{\theta}(\z_i)))),
\end{align*}
for samples $\x_1,\dots,\x_k$ and sampled noise $\z_1,\dots,\z_k\sim\mathcal{N}(\z;0,I)$, while the generator makes use of
\begin{align*}
    \mathcal{L}_G
    &=
    -\E_{\Z\sim\mathcal{N}(0,I)}\l[\log(D_{\phi}(G_{\theta}(\Z)))\r]\\
    &=\approx
    -\frac{1}{k}\sum_{i=1}^k\log(D_{\phi}(G_{\theta}(\z_i)))
\end{align*}
for sampled noise $\z_1,\dots,\z_k\sim\mathcal{N}(\z;0,I)$.
...
\subsubsection{\NEXT Flavours of GANs}
...
Despite their many flavours, GANs come with quirks that other model avoid:
\begin{itemize}
    \item[--] Training them is often a headache, e.g. the discriminator can quickly overfit making the generative side of things diffcult. This necessitates a lot of data.
    \item[--] Mode collapse, in which the diversity of samples at inference, is low.
    \item[--] No approximate likelihood computations according to a GAN so evaluation is difficult.
    \item[--] Training stability is very architecture-dependent, interestingly.
\end{itemize}
...
% equivalence with compression
\subsection{\TODO Generative Modelling $\approx$ Compression?}
\dots

% model evaluation
\subsection{\NEXT Evaluating Generative Models}
Evaluating a supervised learning model is simple enough: leave aside a validation/test set and see how well prediction is done by the model. For generative models, evaluation is a lot less straightforward, e.g. generated samples are not labeled, \dots? Also, what might under/overfitting mean for a generative model? Is the distribution of classes of samples representative of the underlying distribution or do they concentrate on an over-represented subset of classes?

\subsubsection{Density estimation}
May correspond to evaluations of the learned joint probability function $p_{\Theta}(\x)$.

\subsubsection{Sampled images}
\dots
% \textcolor{red}{Future note: A nice method for diffusion models (and beyond) is to take a sample at inference, embed it then compute the $k$ nearest neighbours in said embedding space. Then, by eye, compare the inference sample against its nearest neighbours to see if it's simply reproducing training samples.}

\subsubsection{Sampled text}
\dots
...
\section{Probability Theory \& Statistics Things}
...
% misc. thoughts
\section{Miscellaneous Thoughts}
Here are some thoughts on non-technical things relevant to machine learning. Mostly things I vaguely thought about every now and again but had never put together in writing. The ethics of machine learning is somewhat relevant to some of these but I try to avoid its inclusion. RIP Elaine Herzberg.

% reasoning
\subsection{LLM reasoning [Summer 2024]}
...
Right now, I'm unaware of any machine learning model that can `reason' by any agreed upon definition of the term. I suspect that while current models can simulate `reasoning' to the extent of being able to effectively solve tasks which require some sort of underlying understanding, they do not `reason' in the human sense, which is grounded in embodied experience. Let's say that an LLM (e.g. GPT-4o) concludes that a continuous real-valued function $f:\R\rightarrow\R$ has a root in some interval $(a, b)$ using the intermediate value theorem (IVT) by noting that $f(a)<0$ and $f(b)>0$. Did the LLM `reason' in its application of the IVT or do we instead brush this off as the LLM having been trained on many examples of the application of the IVT? Does it `understand' what it's concluding? As of right now I'd say that it doesn't. This of course isn't something unique to the IVT — we can construct many such examples. What I think makes the IVT interesting is that we can effectively express what it means to `understand' its correctness according to our embodied experience. I'd say that the distinction between the LLM's `understanding' and a human's here is that a human knows that if they trace their finger on a piece of paper starting from the bottom half, crossing a horizontal line cutting through the middle of the paper, to the top half of the paper without removing their finger then their finger must have touched the horizontal line at some point. No mathematical training is needed to know \textit{that} and, more importantly, understand \textit{why} your finger crosses through the horizontal line on the page. LLMs certainly do not `understand' the correctness of the IVT in this way, or really any way that isn't purely symbolic. Maybe multimodal LLMs will alleviate this.

A separate but related question is to what extent it matters that a model `reasons' in order to arrive at its conclusions. Somewhat in the same way that it doesn't matter that a calculator has no inherent idea of why it outputs 6 when you input $3\cdot2$. From a purely problem solving point of view, these philosophical debates regarding what constitutes reasoning, understanding and knowing aren't too important in the short term. Regardless, it'd be funny if 50 years from now we're well-taken care of by a fleet of machine learning-powered robots and we still look down on them for not being capable of `real reasoning'.

\subsection{Backwards-compatible definition of AI [Winter 2026]}
As a term, AI has been thrown around so much that I'm unsure what's meant by it. As of now, early 2026, its intended meaning in most cases is something aking to one of: deep learning, deep generative modelling or LLM-related things. This is somewhat of a shame as its use as a synonym for any of these discludes decades of work which was once referred to as AI. Going all the way back to 1955 when John McCarthy\footnote{John McCarthy was one of the godfathers of AI along with Marvin Minsky, Alan Turing, Allen Newell and Herbert Simon. I'm sure many would argue for the inclusion of others in this list. Also, side note, referring to Geoffrey Hinton as one of the godfathers of AI seems outright wrong. I think it's better to instead refer to Hinton as one of the godfathers of deep learning.} wrote a proposal for the funding of a summer school to be held in 1956 at Dartmouth college. In the proposal he coined the term artificial intelligence to encompass the work of him and his colleagues. Said work was intent on immitating human (or more broadly living) intelligence.

Since 1956, the methods developed and applied with the intention of such immitation have changed drastically, from hard-encoding rules to letting data speaking for itself and allowing models to learn their own rules from data. Regardless, both have been developed and applied with the intention of said immitation, and both, during their time, were understandly referred to as methods belonging to the discipline of artificial intelligence. On this note, I think a definition of AI, as a discipline, which I think can be broadly agreed upon is one which ties it to the intention of immitating the intelligence of the living. The key of this definition is the inclusion of intent. For example, linear programming would not be referred to as an AI approach nowadays but many decades ago there were cases where it certainly was. I believe that a way to distinguish between cases where it should come under the AI umbrella is whether or not the intent of its application was immitating living intelligence.

This definition isn't particularly catchy, so I wouldn't expect it to catch on, but I like that it doesn't disregard a ton of the work done in AI from 1956 to now.

\subsection{\NEXT The principled measure-explainability tradeoff}

\subsection{\TODO The sufficiency of internet-scraped data}
After playing around with large language modelling, e.g. training a $\sim750$ million parameter LLM, and seeing them work (to an extent), a natural question is why on earth internet-scraped text data is sufficient in learning such models.
