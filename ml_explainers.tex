...
\subsubsection{\TODO Grokking}
Unrelated to double descent but another strange behaviour observed during training (sometimes). Welch Labs has a great video on the topic: \url{youtu.be/D8GOeCFFby4?si=4dkxyBbxwYh24W8z}.
...
Transformers are a relatively modern architecture and are relatively broad in their applicability. They're found in language models (e.g. GPTs), image models (vision transformers), denoising models (diffusion) and so on. They offer many advantages over earlier competing architectures, e.g. over RNNs by having input sequence length not being a bottleneck.

The processing of a given input through a transformer made up of a single transformer block involves the following steps
$$
\texttt{tokenisation}
\rightarrow
\texttt{embedding}
\rightarrow
\texttt{position encoding}
\rightarrow
\texttt{attention}
\rightarrow
\texttt{MLP}
$$
each of which will be detailed in this subsection\footnote{Great visualisation: \url{www.poloclub.github.io/transformer-explainer/}.}.
...
Tokens can be thought of as units of information for a given data structure and tokenisation is simply the process of slicing up a sample into the tokens that make it up. Think of splitting sentence `The chef cooked a meal for the critic.' according to the following colouring
$$
...
$$
or slicing up the following image: \textcolor{red}{get a pic here.} To play around with one of OpenAI's text tokenisers, consider \url{platform.openai.com/tokenizer}.

The purpose of tokenisation is self-explanatory: we'd like an idea of dependence between different parts of the input as to aid our output prediction. Notationally, let $V$ denote the set of all tokens in our model's vocabulary and $T:\Sigma^*\to \{1,\dots,|V|\}^*$ the tokenisation function where $S^*$ is the set of all finite strings whose characters are in $S$. It's often helpful in practice to represent the tokenisation of a sequence in terms of a matrix whose columns are one-hot encodings according to $T(\text{input})$.

\subsubsection{\TODO Embedding}
A linear embedding map $L_E:\{1,\dots,|V|\}^*\to\mathbb{R}^d$, where $d\in\mathbb{N}$ is the embedding dimension, offers high dimensional numeric representations of tokens. What makes token embedding functions interesting is how well they can capture underlying semantics, e.g. of natural language. Such an embedding map learned on pieces of text, like Word2Vec, might satisfy
$$
...
L_E(\text{Rome}).
$$
% Suppose the tokenisation of the input consists of $n$ tokens, so $\text{len}(T(\text{input}))=n$ where the output of $T$ is thought of as a list. With this in mind, the embedding of $T(\text{input})$ can be one-hot encoded to obtain the matrix $I\in\{0,1\}^{n\times |V|}$. From here, to embed 

\subsubsection{Positional encoding (for text, learned absolute positional)}
...
\begin{itemize}
    ...
    \item \textbf{Ok, then why not add the index divided by the sequence length to ensure getting something in [0,1]?} For sequences of varying length, we'd like the positional information for a given index to be the same. So attributing 0.5 to the third token in a sequence of five would be an issue as it would also be attributed to the 11th token in a sequence of 20.
    \item \textbf{Why not encode via the binary index?} Change in positional encoding should be smooth. We want $||P_{k+1}-P_k||$ to be bounded but for binary encodings it is simply equal to the square root of the number of bit flips: unbounded in $k$.
\end{itemize}
To address both issues, the authors of the now famous Attention Is All you Need proposed alternating sinusoids. That is, the positional encoding $P_k$ of the $k^{\text{th}}$ token is given by
$$
\l(P_k^{(0)},P_k^{(1)},\dots,P_k^{(d_{\text{model}-1})}\r)
\in
\R^{d_{\text{model}}}
$$
where $d_{\text{model}}$ is the embedding dimension and
\begin{align*}
    P_k^{(2i)}
    &=
    \sin\l(\frac{k}{10000^{\frac{2i}{d_{\text{model}}}}}\r)\\
    P_k^{(2i+1)}
    &=
    \cos\l(\frac{k}{10000^{\frac{2i}{d_{\text{model}}}}}\r)
\end{align*}
for $i=0,\dots,d_{\text{model}}/2-1$. We immdeiately see that $P_k$ is independent of the length $L$ of the input sequence and that the magnitude of $|P_k|$ can be no greater than $d_{\text{model}}$.

Regarding the dynamics of the sinusoid waves, we see that greater indeces $i$ yield the sinusoids of greater period, i.e. lower frequency waves. The importance of this is best understood by example: $P_k^{(0)}=\sin(k)$ has period $2\pi$ and so for $k\in\{1,\dots,L\}$ the first element of $P_k$ varies heavily. At the other extreme
$$
P_k^{(d_{\text{model}}-2)}
=
\sin\l(\frac{k}{10000^{\frac{d_{\text{model}}-2}{d_{\text{model}}}}}\r)
\approx
\sin\l(\frac{k}{10000}\r).
$$
For $k\in\{1,\dots,L\}$, the second to last element of $P_k$ (the final element corresponding to a $\sin$ wave) is practically a straight line whose maximum is close to 0. Both are illustrated in \autoref{fig:bin-and-sin-signals}. As a result, said first element is practically invariant to the token's index $k$. One sees immediately sees the idea being conveyed: the extent to which the index of the token in the sequence influences each element of the positional encoding vector decreases as the index increases. That is, the variance of $P_k^{(i)}$ in $k\in\{1,\dots,L\}$ decreases with respect to $i\in\{1,\dots,d_{\text{model}}\}$. Due to this, these `earlier' dimensions tend to encode finer local positional differences while `later' dimensions encode more global structure.

\begin{table}
    \begin{center}
        \begin{tabular}{cccccccc|c}
            0 & 1 & 2 & 3 & 4 & 5 & 6 & 7 & period\\
            \hline
            0 & 1 & 0 & 1 & 0 & 1 & 0 & 1 & 2\\
            0 & 0 & 1 & 1 & 0 & 0 & 1 & 1 & 4\\
            0 & 0 & 0 & 0 & 1 & 1 & 1 & 1 & 8\\
        \end{tabular}
    \end{center}
    \caption{Bit-reversed binary representations of sequence indeces.}
    \label{tab:binary-encodings}
\end{table}

\subsubsection*{Continuous analog of binary?}
For a comparison of sinusoidal positional encoding to binary encoding, consider \autoref{tab:binary-encodings}. We observe a very similar behaviour: one end of the binary representation seems to correspond to something with a period and going in the other direction we see periodic behaviour with a greater period, i.e. lowe frequency. Transitions with respect to integer (or position) are discrete in this case.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{./figures/neural_nets/binary-signals.pdf}
    \hspace{20pt}
    \includegraphics[width=\columnwidth]{./figures/neural_nets/sin-signals.pdf}
    \caption{Binary signals (top) and sin signals (bottom).}
    \label{fig:bin-and-sin-signals}
\end{figure}

\subsubsection{Attention}
Attention has been around for a while.

\subsubsection{MLP component}
...
\subsubsection{Final transformer block output}
\dots

\subsubsection{Example transformer with numbers}
\dots
...
\textcolor{red}{More broadly, we're interested in modelling some data-generative process so that we may sample from it or reason about the frequency of (incomplete) samples. Easy example is histogram plotting. Where there's frequency-based reasoning there's probabilistic reasoning, hence generative probabilistic modelling.}

With generative probabilistic models, we seek to represent the joint probability function over a set of model variables of interest. Typically, said variables are those which `gave rise' to the data from which the models are learned and, ideally, said models would help to answer probabilistic queries, either exactly or approximately, given new full or partial data.

Put more precisely, what we further refer to as a generative probabilistic model is a tuple $(\X, p_{\Theta})$ consisting of model variables $\X=(X_1,\dots,X_m)$ and a probability function $p_{\Theta}:\Omega_{\X}\rightarrow\mathbb{R}_{\geq0}$ parameterised according to the parameter set $\Theta$. Given a generative probabilistic model, we might hope to answer evidence queries $p_{\Theta}(\x)$, marginal queries $p_{\Theta}(\x')$, where $\x'$ is a realisation of some subset of the model variables, and sampling queries $\hat{\x}\sim p_{\Theta}(\x)$, in which one obtains a `convincing' realisation $\hat{\x}\in\Omega_{\X}$ of the model variables.

The holy grail of generative probabilistic modelling is to find an exact, efficiently computable and algebraically manipulable representation of the underlying joint probability function of interest. If obtained, one can reason probabilistically with ease but, as we will see, the purpose, limitations and learning of generative probabilistic models varies heavily. To illustrate this, consider probabilistic graphical models (PGMs) which offer human-interpretable representations of conditional (in)dependencies between model variables are expressed in the form of graphs. The most well-known class of PGMs are Bayesian networks.

Note that while this section consists of methods of learning generative models pertaining to image-related distributions, the discussed methods are often applicable to generative modelling in general.

% hop nets and boltz machines
\subsection{\TODO Boltzmann machines}
...
A natural approach to representing the joint probability function of model variables is to decompose it into smaller probability functions which are modelled in conjunction. For example, you might decompose the probability function according to the chain rule of probability as in
\begin{align*}
    p(x_1,\dots,x_m)
    &=
...
\begin{example}
    ...
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    Notice that the choice of $X_1$ as the model variable upon which nothing is conditioned is entirely arbitrary. As a result, for a given decomposition, DAGs are not unique. Here, $\theta=(\theta_1,\theta_2,\theta_3)$ are parameters learned from data.
\end{example} 

\subsubsection{\TODO Learning $\mathcal{G}$ and $\theta$ from data}
Unlike with neural networks, the structure of a Bayesian network (its DAG) can be motivated in a principled manner via maximum likelihood estimation. In brief, the likelihood of a DAG can be computed, and so a most likely DAG can be computed. It should be made clear that no analytic solution for the MLE for the DAG exists. As a result, methods typically involve exploring the graph space in a heuristic manner, such as hill climbing.

For parameter learning \dots

\subsubsection{\TODO Inference complexity and tree-width}
Perhaps the most natural question when shown a new generative probabilistic model class is the following: how might it be applied to probabilistic inference, be it exact or approximate? As for exact inference, Bayesian networks suffer a major pitfall due to the complexity of answering marginal queries being exponential in their tree-width, which will be discussed shortly. For example, variable elimination, a widely-used algorithm for answering marginal queries using Bayesian networks, has complexity $O(n\cdot\exp(w))$ where $n$ is the number of nodes and $w$ is the tree-width of the Bayesian network. As such, to appreciate the hardness of answering marginal queries in this context, an understanding of the tree-width of a Bayesian network is needed.

When I first learned about tree-width, it felt like reading definition after definition but the overall intuition was understood easily. We begin with defining tree-decompositions of undirected graphs. A tree-decomposition of an undirected graph $\mathcal{G}$ is a tree $T$ in which nodes pertain to subsets of the nodes in $\mathcal{G}$ and the following three conditions are satisfied:
\begin{enumerate}
    \item Each node of $\mathcal{G}$ appears in at least one of the subsets pertaining to the nodes of $T$.
    \item If there is an edge between two nodes of $\mathcal{G}$ then there is at least one node of $T$ which pertains to a subset containing both.
    \item If a node of $\mathcal{G}$ belongs to the subsets pertaining to two nodes of $T$ then it belongs to all nodes in the path between them.
\end{enumerate}
The width of a tree-decomposition is the cardinality of its largest node minus 1. Tree-decompositions and their width are illustrated in \autoref{fig:tree_decomp}. To lay another definition down, the tree-width of an undirected graph is the minimum width of all of its possible tree-decompositions.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{./figures/generative_models/tree-decomp.jpeg}
    \caption{\TODO make own figure}
    \label{fig:tree_decomp}
\end{figure}

\begin{tcolorbox}[title={\centering\textbf{Intuition check}}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
    The motivation for this construct is \dots
\end{tcolorbox}

Finally, we get to defining the tree-width of a DAG: the tree-width of its moralisation. The moralisation of a DAG $\mathcal{G}$ is the undirected graph whose nodes are that of $\mathcal{G}$ with some directed edges removed. The moralisation has an undirected edge between two nodes if there's an edge between them in $\mathcal{G}$ or if both nodes share a child in $\mathcal{G}$.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{./figures/generative_models/DAG-moralisation.png}
    \caption{\TODO make own figure. also, what is triangulation?}
    \label{fig:DAG_moralisation}
\end{figure}

Such a moralisation captrues all dependencies, direct or not, but often loses some conditional independencies illustrated in $\mathcal{G}$.

\begin{example}
    Consider the Bayesian network below \dots
\end{example}

\subsubsection*{Variable elimination}
\url{https://www.cs.upc.edu/~larrosa/MEI-CSI-files/BN/2-BN-VE.pdf}\\
...
\subsubsection*{Approximate inference: belief propagation}
...
\begin{tcolorbox}[title={\centering\textbf{Why aren't Bayesian nets employed in deep learning?}}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
    Learning their structure and parameters for high-dimensional distributions is entirely uneralistic in terms of compute. Regardless of this `weakness', they serve as a significant, and often overlooked, step in the evolution of probabilistic reasoning and, more broadly, generative probabilistic modelling.
\end{tcolorbox}
...
\subsection{\IMPROVE The Curse of Dimensionality}
...
\subsection{\TODO Quality of fit $\approx$ Encoding quality?}
