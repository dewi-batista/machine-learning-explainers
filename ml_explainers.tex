...
$$
which pertains to taking $d(f_{\theta}(\x_i), y_i)=(y_i-f_{\theta}(\x_i))^2$. Note that the factor of $1/n$ is often left out when discussing MSE as minimising the expression in $\theta$ is invariant to the inclusion of the factor — it's mostly a notational convenience thing. So in our case, we seek to compute
$$
...
$$
f_{Y|\X}(y_i|\x_i;\theta)=\frac{1}{\sqrt{2\pi}\sigma}\exp\l(-\frac{(y_i-f_{\theta}(\x_i))^2}{2\sigma^2}\r).
$$
The maximum likelihood estimator $\theta_{\text{MLE}}$ for the parameters of our linear regression model is that which maximise our log-likelihood. That is,
\begin{align*}
    \theta_{\text{MLE}}
    &=
    \argmax_{\theta\in\R^{p+1}}\l[\log\l(\prod_{i=1}^n f_{Y|\X}(y_i|\x_i;\theta)\r)\r]\\
    &=
    \argmax_{\theta\in\R^{p+1}}\l[\sum_{i=1}^n\log(f_{Y|\X}(y_i|\x_i;\theta))\r]\\
    &=
    \argmax_{\theta\in\R^{p+1}}\l[n\log\l(\frac{1}{\sqrt{2\pi}\sigma}\r)-\frac{1}{2\sigma^2}\sum_{i=1}^n(y_i-f_{\w,b}(\x_i))^2\r]\\
    &=
    \argmin_{\theta\in\R^{p+1}}\l[\sum_{i=1}^n(y_i-f_{\w,b}(\x_i))^2\r].
\end{align*}
...
\begin{table}[ht]
    ...
    \end{center}
\end{table}
Since $\Dtrain$ consists of only five samples, it's feasible to compute the optimal parameters $\thetaOpt$ by hand. We have
$$
y=
\begin{bmatrix}
    180\\
...
$$
where the sum is taken over the training data $\{(\x_i,y_i)\}_{i=1}^5$.
...
\begin{center}
    \textbf{\textcolor{blue}{Q1:} What are the benefits of using logistic regression over more sophisticated methods?}
\end{center}
...
\begin{center}
    \textbf{\textcolor{blue}{Q2:} Which loss function is appropriate for gradient descent?}
\end{center}
...
\begin{center}
    \textbf{\textcolor{blue}{Q4:} How should we initialise the parameters? \textcolor{red}{TODO}}
\end{center}
...
Onto the derivation, which makes use of the notation introduced in \autoref{sec:multi_layer_perceptrons}. The dataset from which the MLP is learned is constant, and so in deriving backpropagation the loss function is treated as a function of the weights and biases only, i.e.
$$
\mathcal{L}\l(\mathbf{W}^{(1)},\dots,\mathbf{W}^{(k+1)},\mathbf{b}^{(1)},\dots,\mathbf{b}^{(k+1)}\r)
$$
which we denote by just $\mathcal{L}$. Let $\mathbf{z}^{(j)}=\mathbf{W}^{(j)}\mathbf{a}^{(j-1)}+\mathbf{b}^{(j)}$ for $j\in[k+1]$ from which we have $\mathbf{a}^{(j)}=\sigma_{j}\l(\mathbf{z}^{(j)}\r)$. Further, let
$$
\delta^{(j)}_i
=
...
\textcolor{red}{TODO: note that this is just a single-layer perceptron}: most activation functions are monotonic, so single layer perceptrons reduce to, again, a hyperplane in feature space. like the first paragraph states, if we allow for arbitrary non-linear activation functions then this is possbile in theory (but unrealistic). monotonicity kills it

\begin{center}
    \textbf{\textcolor{blue}{Q4:} Must activation functions be monotonic?}
\end{center}
Nope, it just makes things related to optimisation easier while having a sufficiently-small effect on practical performance. Additional minor point: some proofs of the universal function approximation theorem of MLPs necessarite monotonicity but these are typically intentionally short proofs; it isn't neceesary in general.
...
\noindent Through skip connections. Architectures which use them are sometimes called residual networks.
\begin{itemize}
...
Here are some probability theory-related things, e.g. long derivations, which are well-suited for an appendix.
...
\subsubsection{Distribution fitting using maximum likelihood estimation \textcolor{red}{TODO}}
...
\subsection{The Central Limit Theorem (CLT) \textcolor{red}{TODO}}
\dots
...
I don't think about the philosophic side of maching learning very often but I find some topics interesting.
