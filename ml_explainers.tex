...
\begin{figure}[t]
    ...
    \caption{A neural network with one hidden layer ($k=1$). \TODO{: The figure should read `hidden layer', not plural.}}
...
\begin{figure}[t]
    ...
    \caption{An autoencoder in which $\theta$ and $\phi$ are fit using multi-layer perceptrons (MLPs). \TODO{: Colour missing in text? Also change $n$ to $q$}}
...
The intended meaning of the singular `sample' is typically a set of realisations of some random variable. So $\{x_1,\dots,x_n\}\subset\Omega_X$ for some random variable $X$ is a sample. Where things can become confusing is that in machine learning contexts in which a `sample' refers often to refers to a single data point. This was a huge cause of confusion during my first few meetings with my master's thesis supervisor who is a Bayesian statistician.

As for sample independence, suppose we'd like to get an idea of the mean population height of men and women. If we randomly sample 100 men and 100 women then we're good to go. This could be done by independently sampling, without replacement, from the categorical corresponding to the ID numbers of individuals. If we instead randomly sample 100 couples then our samples would not be independent since couples' heights correlate.
...
\end{align*}
If $X$ and $Y$ are independent then $\E[XY]=\E[X]\E[Y]$, i.e. $\text{Cov}(X,Y)=0$, and so
$$
\text{Var}(X+Y)
=
\text{Var}(X)+\text{Var}(Y).
$$
It's worth noting that there are plenty of contexts in statistics in which different measure of distribution spread are more convenient, e.g. the mean absolute deviation about the median $\E[|X-\text{med}(X)|]$ in the context of robust statistics.


\subsection{Estimators, Bias and Variance}
...
\begin{align*}
    ...
    \E[\hat{\mu}]-\mu\\
    &=
    \E\l[\frac{1}{n}\sum_{i=1}^nX_i\r]-\mu\\
    &=
