...
$$
are the feature values of the $i^{\text{th}}$ sample and $y_i\in \OY\subseteq\R$ is the corresponding output. The reason I chose $|D|=2n$ is that it results in being able to 50/50 split $D$ into $D_{\text{train}}$ and $D_{\text{test}}$ with $|D_{\text{train}}|=|D_{\text{test}}|=n$. A linear regression models fits a linear function (in the parameters)
\begin{align*}
...
\end{align*}
where $\theta=(\theta_0,\theta_1\dots,\theta_q)\in\R^{q+1}$ are the model parameters, i.e. the values we can tweak to our heart's desire until the corresponding model is sufficiently well according to some metric. Some people refer to the paramter $\theta_0$, which dictates the elevation of the hyperplane corresponding to $f_{\theta}(\x)=0$, as the bias of the model which I find very confusing as there are a bunch of other intended meanings of the term `bias' in statistics and machine learning. I prefer to refer to it as the elevation. Anyway, once such a linear function has been fit, given feature values $\x\in\OX$ our model predict the corresponding output as $y=f_{\theta}(\x)$.

\begin{tcolorbox}[title={\textbf{Assumption of normally distributed residuals}}, colback=c2, colbacktitle=c13, colframe=c13, coltitle=white]
...
An intuitive approach to finding the `optimal' parameters for a linear regression model, which we denote by $\thetaOpt$, is to split $\D$ into training and testing datasets $D_{\text{train}}$ and $D_{\text{test}}$ (each consisting of $n$ sample in our case) and minimising some pre-determined loss function of said parameters over $D_{\text{train}}$. Essentially, minimising something like
$$
...
\noindent We know that in the context of linear regression, the optimal paramters $\thetaOpt$ are typically taken to be those which minimise the mean square error over $D_{\text{train}}$ but how do we actually compute $\thetaOpt$? This could be done through numerical methods, which is often the case in machine learning, e.g. using gradient descent in computing the optimal parameters of a logistic regression model, but linear regression has a closed form solution. This is pretty cool since it's not so common for such closed form solutions to exist. The informal method which I use to remember the closed form solution for the optimal paramters of a linear regression model is
$$
...
\begin{tcolorbox}[colback=c10]
    \textbf{Note:} Some literature define linear regression models as regression models that are linear in their parameters. By this definition, linear regression, as a term, would encompass polynomial regression models and other regression models with non-linear basis functions. This confuses me as `linear regression' is most often intended to mean models that fit a hyperplane to data.
\end{tcolorbox}
...
\begin{tcolorbox}[title={\textbf{Funny misuse of linear regression: Momentous sprint at the 2156 Olympics?}}, colback=c2, colbacktitle=c13, colframe=c13, coltitle=white]
...
%     \end{center}
% \end{table}
% Since $D_{\text{train}}$ consists of only five samples, it's feasible to compute the optimal parameters $\thetaOpt$ by hand. We have
% $$
% y=
% \begin{bmatrix}
%     180\\
...
% $$
% f_{\thetaOpt}(x_1,x_2)=25.68+0.94x_1+5.79x_2.
% $$
...
% =
% \sum_{i=1}^5 (y_i-25.68-\theta_1\x_{i,1}-\theta_2\x_{i,2})^2
% $$
% where the sum is taken over the training data $\{(\x_i,y_i)\}_{i=1}^5$. \textcolor{red}{TODO: make the loss landscape figure.}

% As for how well the model performs, its MSE on $D_{\text{train}}$ is $1.26$ and its MSE on $D_{\text{test}}$ is $12.24$. In this case, such a difference is fine. To get a visual on how well the model does on the training and testing sets, consider \autoref{fig:lin_reg_example_on_train_and_test}. I should improve these at some point. \textcolor{red}{TODO: offer a figure for each projected onto each of the two feature axes?}
...
%     \caption{Model output for the training set (left) and testing set (right). Hard to appreciate these 3D figures.}
%     \label{fig:lin_reg_example_on_train_and_test}
% \end{figure}

% \textcolor{red}{provide $R^2$ for train and test}
...
\begin{align*}
    ...
    &=
    \sum_{i=1}^n\l[y_i\log\l(\frac{1}{1+e^{-\theta^{\top}\x_i}}\r)+(1-y_i)\log\l(\frac{e^{-\theta^{\top}\x_i}}{1+e^{-\theta^{\top}\x_i}}\r)\r]\\
    &=
...
$$
where $K(\x,\z)=\langle\phi(\x),\phi(\z)\rangle$ is a pre-chosen kernel. See how it never requires an explicit computation involving $\phi$. Hence the name `kernel trick' (not the same kernel trick as in variational autoencoders). For some reason, solving the dual is easier. \TODO{: explain why it's easier.}
...
\TODO{: First give a subsubsection on under and overfitting.}
...
\begin{align*}
    ...
    \textcolor{c11}{\E_{\D}\l[\E_\X\l[\l(f(\X)-\hat{f}_{\D}(\X)\r)^2\r]\r]}\\
...
\E_{\D}\l[R\l(\hat{f}_{\D}\r)\r]
=
\textcolor{c5}{\E_{\D}\l[\E_{(\X,Y)}\l[\l(Y-f(\X)\r)^2\r]\r]}+\textcolor{c11}{\E_{\D}\l[\E_\X\l[\l(f(\X)-\hat{f}_{\D}(\X)\r)^2\r]\r]}
$$
...
\end{align*}
See that the final term vanishes as $\E_{\D}\l[\bar{f}(\x)-\hat{f}_{\D}(\x)\r]=0$ for all $\x\in\OX$ and so
\begin{align*}
    &\hspace{14.52pt}
    \textcolor{c11}{\E_{\D}\l[\E_\X\l[\l(f(X)-\hat{f}_{\D}(\X)\r)^2\r]\r]}\\
    &=
...
\end{center}
\TODO{: Note that it corresponds to MLE for normally distributed residuals.}
...
%\textbf{Batch learning:} Loss landscape gets smoothed out. Make sure to make a note of what Welch Labs states about projecting loss landscapes: every step you make in the actual high dimensional loss landscape will change its 2D projection. His example of the actual loss landscape being 2D and illustrating its 1D projection makes for a great example (he mentions that this only applies to the case of training on a single sample? I don't understand entirely as of now). He also makes an interesting point of ``if we get stuck in a global minimum then it must be a minima wrt all dimensions, which is less likely to happen the higher the dimension'' whatever that might mean precisely.
...
\begin{tcolorbox}[title={\textbf{Autoencoder example}}, colback=c2, colbacktitle=c13, colframe=c13, coltitle=white]
...
\begin{tcolorbox}[title={\textbf{VAE example}}, colback=c2, colbacktitle=c13, colframe=c13, coltitle=white]
...
\subsubsection{Backpropagation for VAEs (reparameterisation trick)}
...
Pearson correlation, as discussed in \autoref{app:pearson_correlation}, is a measure of linear dependence between random variables. What if we want to know about non-linear dependence between random variables, i.e. what if Pearson correlation doesn't suffice? Mutual information to the rescue.
