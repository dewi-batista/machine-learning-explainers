...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{./figures/neural_nets/neural_net_1.pdf}
...
\subsubsection{Universal function approximation theorem (expressivity)}
...
\mathcal{O}((k+1)n^2),
$$
i.e. complexity grows linearly in the number of layers and quadratically in the number of neurons per layer. So we argue for increasing depth over width. Far more parameter-efficient.
...
On top of this, pixel values are very usually correlated, so why use an MLP in which many nodes are redundant?
...
\subsubsection{Example of using entropy}
