...
% \subsubsection{Limitations/hard bottlenecks}
...
\begin{figure}[t]
    \centering
    % \includegraphics[height=6.825cm, keepaspectratio]{./figures/neural_nets/augusto.png}
    % \includegraphics[height=7cm, keepaspectratio]{./figures/neural_nets/augusto-patchified.pdf}
    \caption{The tokenisation (or patchification) of Augusto.}
...
F_{\phi}^{-1}(u_i)
$$
where $u_1,\dots,u_k$ are i.i.d. samples from $U(0,1)$ then the process by which our latents are obtained is not deterministic but explicitly depends on $\phi$. How then does one backpropagate through this process? Auto-differentation isn't a fan. The idea of the reparameterisation trick is very natural: entirely separate the stochasticity obtaining said samples from the parameters $\phi$. This can be done by taking $\Z|\x=g(\epsilon; \phi, \x)$ for invertible $g$ in which $\epsilon\sim\mathcal{N}(0, I)$ is the only stochastic component of sampling. With such a function $Z=g(\epsilon;\phi,\x)$ we have (via the law of the unconscious statistician)
\begin{align*}
    \nabla_{\phi}\E_{\Z\sim q_{\phi}(\z|\x)}[f(\Z)]
    &=
...
\begin{align*}
    ...
    &=
    \nabla_{\phi}\E_{p(\epsilon)}\l[\log(p_{\theta}(\x|\Z))+\log(p_{\theta}(\Z))-\log(q_{\phi}(\Z|\x))\r]\\
    &=
...
\end{align*}
where $\z_i=g(\epsilon_i;\phi,\x)$ and $\epsilon_1,\dots,\epsilon_k\sim\mathcal{N}(\epsilon;0,I_d)$. Ignore my abuse of notation of conflating the random variable $\epsilon\sim\mathcal{N}(0,I_d)$ with corresponding samples. This estimator is unbiased and all that good stuff. For an illustration of how the reparameterisation helps alleviate the earlier issue with backpropagation, consider \autoref{fig:reparameterisation_trick}.

An explicit example of such a reparameterisation is when the posterior is a facorised Gaussian. That is, $q_{\phi}(\z|\x)=\mathcal{N}(\z;\mu_{\phi}(\x), \text{diag}(\sigma_{\phi}^2(\x)))$. In this case, take $g(\epsilon;\phi,\x)=\mu_{\phi}(\x)+\sigma_{\phi}(\x)\odot\epsilon$ where $\epsilon\sim\mathcal{N}(\epsilon;0,I_d)$ and $\odot$ denotes element-wise multiplication. Typically, the prior $p_{\theta}(\z)$ and the likelihood $p_{\theta}(\x|\z)$ are also taken such that their logarithm is nice to compute and backproppable.

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{./figures/generative_models/diffusion_process.pdf}
    \caption{Backpropagation before and after applying the reparameterisation trick.}
...
\begin{tcolorbox}[title={\centering\textbf{Computing} $\log(q_{\phi}(\z|\x))$}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
    Though nice distributions are often attributed to the posterior, it's worth noting that there's a convenient manner of computing $\log(q_{\phi}(\z|\x))$ which relies on the invertibility of $g(\epsilon;\phi,\x)$. Note that
    \begin{align*}
        \log(q_{\phi}(\z|\x))
        &=
        \log\l(p(\epsilon)\l|\det\l(\frac{\partial \z}{\partial\epsilon}\r)\r|^{-1}\r)\\
        &=
        \log(p(\epsilon))-\log\l(\l|\det\l(
        \begin{bmatrix}
        ...
        \r)\r|\r)
    \end{align*}
    where $\epsilon=(\epsilon_1,\dots,\epsilon_d)$. This ``speed-up'' is particularly useful for complicated-looking choices of the posterior.
\end{tcolorbox}
...
\subsubsection{\NEXT Quirks of diffusion models}
\begin{itemize}
    \item[--] \dots
\end{itemize}
