...
Transformers are a relatively modern architecture but they have already been applied very broadly. They're now found in language models (e.g. GPTs), image models (vision transformers), denoising models (diffusion) and so on. They offer many advantages over earlier competing architectures, e.g. RNNs, due to not being bottlenecked by input sequence length.

The processing of a given input through a single transformer block involves
$$
\texttt{attention}
\rightarrow
\texttt{MLP}.
$$
As such, the full processing through a transformer architecture looks something like
$$
\texttt{tokenise}
\rightarrow
\texttt{embed}
...
\subsubsection{Example: GPT-2 small (124M parameters)}
\dots
