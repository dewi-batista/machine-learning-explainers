...
$$
or slicing up the following image: \textcolor{red}{get a pic here.}
...
$$
In the latter sequence, the intended meaning of \colorbox{brown}{cooked} is akin to `criticised', while in the former it is simply meant to convey that the chef literally cooked. So how do transformers go about accounting for the positions of tokens in given input sequences?\footnote{I learned from the following video: \url{www.youtube.com/watch?v=dWkm4nFikgM}.}

Unlike RNNs, transformers have no inherent notion of the order of tokens in a given input. Due to this, sprinkling in position-related information to the embedded input is necessary. Before detailing what was proposed in the paper that made transformers famous, it's worth attending to the validity of arguably natural approaches and the ways in which they fall short:
\begin{itemize}
...
\end{itemize}
To address both issues, the authors of the now famous Attention Is All you Need used sinusoids. Use sinusoids to form a position vector $P_k$ for the $k$th token in an input sequence of length $L$. Add $P_k$ to the word embedding $E_k$ for $k=0,\dots,L-1$ to obtain the input
$$
