...
\begin{align*}
    ...
    \text{ELBO}
\end{align*}
in which the inequality arises due to Jensen's inequality, as in $\E[\log(f(\Z))]\leq\log(\E[f(\Z)])$, and $D_\text{KL}(Q||P)$ denotes the KL-divergence between distributions $Q$ and $P$, which is detailed in the appendix. It's worth noting that equivalent but different-looking representations of the ELBO exist and are common in literature, as in
\begin{align*}
    \text{ELBO}
    &=
    \E_{\Z\sim q_{\phi}(\z|\x)}\l[\log(p_{\theta}(\x|\Z))\r]-\E_{\Z\sim q_{\phi}(\z|\x)}\l[\log\l(\frac{q_{\phi}(\Z|\x)}{p_{\theta}(\Z)}\r)\r]\\
...
% \subsubsection{\TODO Backpropagation for VAEs (the reparameterisation trick)}
