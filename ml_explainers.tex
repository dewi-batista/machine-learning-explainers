...
Regarding the dynamics of the sinusoid waves, we see that greater indeces $i$ yield the sinusoids of greater period, i.e. lower frequency waves. The importance of this is best understood by example: $P_k^(0)=sin(k)$ has period $2\pi$ and so for $k\in\{1,\dots,L\}$ the first element of $P_k$ varies heavily. At the other extreme, $P_k^(d_{\text{model}}-2)=\sin\l(\frac{k}{10000^{\frac{d_{\text{model}}-2}{d_{\text{model}}}}}\r)\approx\sin(\frac{k}{10000})$. For $k\in\{1,\dots,L\}$, the second to last element of $P_k$ (the final element corresponding to a $\sin$ wave) is practically a straight line. Both are illustrated in \autoref{fig:sin-functions}. As a result, said first element is practically invariant to the token's index $k$.
