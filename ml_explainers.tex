...
% hyperparam tuning
\subsection{\REV{: }Hyperparameter Tuning}
...
\textbf{Benchmark overfitting:} Consider the following scenario: group A and group B independently publish results regarding almost-identical MLP architectures, applied to the same task, whose only distinction is the number of neurons in one of the hidden layers of the latter being one less than the former. Every aspect of their training is identical: their training data, how they split it in order to perform validation, etc. Group A's MLP obtains a lower test error than group B and so those developing commercial applications that require an MLP choose group A's. How is this different from a single group selecting the former's MLP architcture over the latter's by simply peeking at their test loss? That is, choosing hyperparameters on the basis of test loss.

The distinction is about who had access to the test set during design:

Single group scenario:
If you train both versions (100 vs 200 neurons) and then pick the one with lower test error, you've let the test set influence your design. That makes your final test error estimate biased (too optimistic), because the choice was tailored to that specific test set.

Two independent groups scenario:
Each group is supposed to have done model selection using validation data only, and then evaluated once on the test set. Their reported test errors are (in expectation) unbiased estimates of generalization.
When you compare their numbers, you're not re-tuning a model — you're just picking between two already-fixed systems.

The subtlety: the test set's role is about contamination. If a single team peeks at it during design, it contaminates their result. But if two groups independently publish their final test results, the contamination risk (for each group individually) is assumed to be small, and as a consumer you can compare them.

In practice, though, you're right — if the same benchmark test set is used over and over (like CIFAR-10 or ImageNet), the community as a whole is effectively tuning to the test set. That's why people talk about benchmark overfitting and introduce fresh test sets (e.g., ImageNet-v2).

So the rule isn't about “artificial separation,” it's about who touches the test set during decision-making.

Do you want me to explain this in terms of data leakage / overfitting risk instead of research group scenarios? That might make the distinction feel less arbitrary.
...
\begin{figure}%[t]
    \centering
    \begin{minipage}{0.6\linewidth}
        \includegraphics[width=\columnwidth]{./figures/generative_models/AE_gen_1.pdf}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.38\linewidth}
        \caption{Decodings of sampled latents. Gibberish output.}
...
\begin{figure}%[t]
    \centering
    \begin{minipage}{0.6\linewidth}
        \includegraphics[width=\columnwidth]{./figures/generative_models/AE_gen_2.pdf}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.38\linewidth}
        \caption{Decodings of latents close to a given encoding.}
...
P.S. I advise people who are entirely new to probability theory and statistics to consider playing around with interative visual demonstrations of ideas, as in \href{https://seeing-theory.brown.edu/index.html}{\texttt{seeing-theory.brown.edu/index.html}}.
