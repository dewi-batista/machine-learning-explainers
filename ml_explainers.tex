...
$$
The derivation of this statement is boring, so I'll leave it out. Worth noting that it's typically expressed as
$$
R^2
=
...
As for how well the model performs, its MSE on $\Dtrain$ is $1.26$ and its MSE on $\Dtest$ is $12.24$. In this case, such a difference is fine. To get a visual on how well the model does on the training and testing sets, consider \autoref{fig:lin_reg_example_on_train_and_test}. I should improve these at some point. \textcolor{red}{TODO: offer a figure for each projected onto each of the two feature axes?}
...
I know pretty much nothing about these ;(
...
\end{center}
If there were a single activation function towards the end of the architecture then everything that came before would be a series of linear transformations of the input. The composition of linear transformations is just a linear transformation. As such, said model would be equivalent to a single layer perceptron, i.e. only linear decision boundaries.

To be super pedantic, single-layer perceptrons yielding only linear decision boundaries is the case if the activation function used is monotonic, which is often the case. If you allow for weird, non-monotonic activations then this linear decision boundary result doesn't necessary hold but, again, this is pedantic to point out.
...
% autoencoders
\subsubsection{Autoencoders: no variation!}
...
\textcolor{red}{TODO: not to be pedantic but Pearson correlation is really a property of a joint distribution over X and Y, not of the RVs themselves; distributions over X and Y individaully are marginals...}
...
\subsection{Mutual Information \textcolor{red}{TODO: belongs in prob theory section?}}
...
\subsection{Clarifying likelihoods, especially MLE}
