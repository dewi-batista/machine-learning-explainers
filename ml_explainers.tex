...
This is precisely the idea that motivates GANs with inspiration taken from game theory. A GAN consists of a generative component $G:\Omega_{\Z}\to\Omega_{\X}$ and a discriminative component $D:\Omega_{\X}\to\{0,1\}$, which are trained alternately in the style of a minmax game. That is, $G$ produces a new sample given a latent $\z\in\Omega_{\Z}$ while $D$ learns to distinguish between training samples and outputs of $G$. Essentially, the models compete. In terms of loss functions, the discriminator employs binary cross entropy, as in
$$
\mathcal{L}_D
=
...
-\E_{\Z}\l[\log(D(G(\Z)))\r].
$$
As for architectures, the usual choices are natural: the disrciminator is fit using a CNN and the generator is fit using a transposed CNN. So, as with other image generative models, to generate a new sample, we sample Gaussian noise and run it through the transposed CNN.
...
I wish my understanding of probability theory, measure theory, stochastic processes, statistics (classical, Bayesian and asymptotic) were better. In another life.
...
% Perhaps worthy of its own subsection in the main body, unsure
