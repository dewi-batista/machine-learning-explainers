...
The two simplest kernels are polynomial kernels and the RBF kernel. Polynomial kernels are of the form \dots \TODO{: Complete.}
...
Here are some probability theory and statistics-related things, e.g. brief statements, long derivations, and so on, which are better-suited for an appendix. I wish I knew more probability/measure theory and classical statistics. In another life.
...
f(\E[X])\geq\E[f(X)].
$$
I've never come across a particularly interesting proof for it, so I won't include one.
...
It turns out that there's a clever way of perfmorming MLE while accounting for hidden variables called the expectation-maximisation (EM) algorithm. It involves obtaining a lower bound for the log-likelihood of a given sample and instead maximising the lower bound. If said bound can be made into an equality then the application of EM is MLE itself. If the bound is not exact then we refer to the process as variational EM.

In obtaining said lower bound, let $\theta$ denote the parameters of the true joint $p_{\theta}(\x,\z)$. If $q$ is a distribution over $\Omega_{\Z}$ then
\begin{align*}
    \log(p_{\theta}(\x))
    &=
    ...
    \text{ELBO}(q,\theta).
\end{align*}
The EM algorithm seeks to perform coordinate ascent (component-wise equivalent of gradient ascent): iteratively maximise $\text{ELBO}(q,\theta)$ in $q$ and then $\theta$ until convergence. With $q$ fixed, \textbf{maximising the ELBO in} $\theta$ amounts to computing
\begin{align*}
    ...
    \argmax_{\theta}Q(\theta)
\end{align*}
where $Q(\theta)=\E_{\Z\sim q}\l[\log\l(p_{\theta}(\x|\Z)\r)\r]+\E_{\Z\sim q}\l[\log\l(p_{\theta}(\Z)\r)\r]$. Note that equality is offered by the bound precisely when $q(\z)=p_{\theta}(\z|\x)$ and so fixing the parameters and \textbf{maximising the ELBO in} $q$ amounts to computing
$$
\argmax_{q}\text{ELBO}(q,\theta)
=
p_{\theta}(\z|\x),
$$
i.e. matching the true posterior.

With these in mind, the EM algorithm involves initialising the parameters $\theta^{(0)}$ and repeating the following two steps until some pre-determined stopping criteria is met:
\begin{enumerate}
    \item (E step) Obtain a closed form for $q_t(\z)=p_{\theta^{(t)}}(\z|\x)$ and in turn a closed form for $Q(\theta)$ (the expectations are with respect to $q_t$)
    \item (M step) Compute $\theta^{(t+1)}=\argmax_{\theta}Q(\theta)$
\end{enumerate}
...
\begin{tcolorbox}[title={\centering\textbf{Variational EM}}, colback=myLightBlue, colbacktitle=myDarkBlue, colframe=myDarkBlue, coltitle=white]
    Computing the posterior exactly is often infeasible. \textbf{Varational EM} alleviates this difficulty by restricting $q$ to a family of functions $\mathcal{F}$, e.g. some family of MLPs, and replacing the E-step with the computation
    $$
    q_t
    =
    ...
    $$
\end{tcolorbox}
Regarding the \textbf{correctness} of the EM algorithm, we seek whether or not this coordinate ascent method converges \TODO{: does this need more?}, i.e.
$$
\log(p_{\theta^{(t)}}(\x))
\leq
\log(p_{\theta^{(t+1)}}(\x))
$$
for all $t\in\mathbb{N}$. To see this, note that
$$
\log(p_{\theta^{(t+1)}}(\x))
\geq
...
\log(p_{\theta^{(t)}}(\x)).
$$
The leftmost inequality is simply by how the ELBO is defined. The second inequality is due to
$$
\theta^{(t+1)}
=
\argmax_{\theta}\text{ELBO}\l(q_t,\theta\r).
$$
The final inequality follows from the `match the true posterior' argument above.
