...
\subsubsection{\NEXT Positional encoding}
...
$$
In the latter sequence, the intended meaning of \colorbox{brown}{cooked} is akin to `criticised', while in the former it is simply meant to convey that the chef literally cooked. So how do transformers go about accounting for the positions of tokens in given input sequences?\footnote{Great video about this topic: \url{www.youtube.com/watch?v=dWkm4nFikgM}.}

Unlike RNNs, transformers have no inherent notion of the order of tokens in a given input without this additional positional encoding step. That is because the attention and MLP components are, standalone, independent of position. Due to this, sprinkling in position-related information to the embedded input is necessary. I like to think of positional encoding as a sort of prior regarding which tokens are relevant to others. That is, the prior is relevant but contextual information baked into the text itself will override its inclusion if sufficiently relevant. Form a position vector $P_k$ for the $k^{\text{th}}$ token in the input sequence of length $L$. Add $P_k$ to the word embedding $E_k$ for $k=0,\dots,L-1$ to obtain
$$
(E_0+P_0,\dots,E_{L-1}+P_{L-1})
$$
fed to the model.

Before detailing what was proposed for positional encoding in the paper that made transformers famous, it's worth attending to the validity of arguably natural approaches and the ways in which they fall short:
\begin{itemize}
    ...
    \item \textbf{Why not encode via the binary of the index?} Change in positional encoding should be smooth. We want $||P_{k+1}-P_k||$ to be bounded but for binary encodings it is simply equal to the square root of the number of bit flips: unbounded in $k$.
\end{itemize}
To address both issues, the authors of the now famous Attention Is All you Need proposed alternating sinusoids. Sinusoidal positional encoding feels unnatural to me, though it has a nice motivation as a continuous analog of binary encoding. Regardless, far more intuitive methods of positional encoding exist such as learned positional encoding (used by OpenAI for GPT-2) and the now-popular rotary positional embeddings (RoPE).

Learned positional encoding is just as the name suggests: the positional embedding map is learned according to backpropagation like any other parameter. RoPE is bit more involved, relying on the linearity of rotations in $\R^d$.

\subsubsection{Attention}
Stand at attention!

\subsubsection{MLP component and final block output}
On top of all detailed so far, transformer blocks consist of an MLP component. The input of said MLP component is the output of the attention component.

\subsubsection{Example: GPT-2 small ($\sim$124M parameters)}
\dots
...
% (deep) generative models (all other section names i've come up with were overthought)
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{./figures/generative_models/histogram.pdf}
    \caption{A histogram pertaining to the frequency of weather states. Corresponding probabilities (post-normalisation) in parentheses.}
    \label{fig:histogram}
\end{figure}
...
% % TODO: move to selected-topics-explainers
% \section{Applying the Chinese Remainder Theorem}
% Given a system of integer congruences
% \begin{align*}
%     k & \equiv k_{j_1}\phantom{1}(\mathrm{mod}\phantom{1}j_1)\\
%       & \vdots\\
%     k & \equiv k_{j_l}\phantom{1}(\mathrm{mod}\phantom{1}j_l)
% \end{align*}
% in which $j_1,\dots,j_l$ are pairwise coprime (i.e. $\gcd(j_r,j_s)=1$ for $r\neq s$), it is of interest to recover $k$.

% Note that by the CRT, solutions of such integer congruence systems are unique up to congruence modulo $J:=j_1\cdots j_l$. That is, if $x_1$ and $x_2$ satisfy all congruences then $x_1\equiv x_2\phantom{1}(\mathrm{mod}\phantom{1}J)$. With this in mind, if we can construct some $k'$ such that all congruences are satisfied then we have that $k'\equiv k\phantom{1}(\mathrm{mod}\phantom{1}J)$, i.e. we will have recovered the remainder of $k$ modulo $J$. This application of the CRT is important to solving the discrete logarithm problem over elliptic curve groups. A natural question here is how one constructs $k'$.

% Consider $k'=\sum_{i=1}^l k_{j_i}e_{j_i}$ where $e_{j_i}$ satisifies
% \begin{align*}
%     e_{j_i} & \equiv 1\phantom{1}(\mathrm{mod}\phantom{1}j_i)\\
%     e_{j_i} & \equiv 0\phantom{1}(\mathrm{mod}\phantom{1}j_s) \text{ for }s\neq i
% \end{align*}
% for $i=1,\dots,l$. With such a construction, one has $k'\equiv k_{j_i}\phantom{1}(\mathrm{mod}\phantom{1}j_i)$ for $i=1,\dots,l$ and so the CRT yields
% $$
% k
% \equiv
% \sum_{i=1}^l k_{j_i}e_{j_i}\phantom{1}(\mathrm{mod}\phantom{1}J)
% $$
% where $J=j_1\cdots j_l$. Then the difficulty of constructing $k'$ comes down to constructing $e_{j_1},\dots,e_{j_l}$ which turns out not to be too difficult: take $e_{j_i}=J_iy_i$ where $J_i=J/j_i$ and $y_i$ is the remained of $J_i^{-1}$ modulo $j_i$. Note that the existance of $y_i$ is guaranteed due to $\gcd(j_i,J_i)=1$. Bam, done, simply take
% $$
% k'
% =
% \sum_{i=1}^l k_{j_i}J_iy_i.
% $$
% So finding the remainder of $k$ modulo $J$ boils down to computing the $l$ remainders $y_1,\dots,y_l$ of $J_1^{-1},\dots,J_l^{-1}$ modulo $j_1,\dots,j_l$ respectively.
...
\subsection{\NEXT The Curse of Dimensionality}
\url{https://stats.stackexchange.com/q/219373/396940}\\
Dimension goes up, amount of data needed to cover to-be-fit manifold grows exponentially. A nice way to remember the curse of dimensionality is the fact that the ratio of the volumes of the unit $n-$ball (always 1) and $n-$box tends to 0, asymptotically: $n=1$ yields a ratio of 2, $n=2$ yields $\pi/4$, $n=3$ yields $\pi/6$, and so on.

Due to this, one might fear that the amount of data required to fit manifolds in these horribly high-dimensional spaces is absurd. This though leads naturally to the instrinsic dimension of such manifolds. Thankfully, this is often not the case for distributions of interest, in practice. Learning said distributions boils down to fitting manifolds embedded in a space whose dimension is far higher than their intrinsic dimension.

\begin{example}
    Consider the situation illustrated in \autoref{ex:autoencoder} in which the object $\{(a,a,b)\in\R^3:||(a,a,b)||\leq1\}$ is of interest. It's simply the intersection of the unit ball and the plane $\{(a,a,b):a,b\in\R\}$. That is, the intrinsic dimension of the object is 2 while the dimension of the space in which it's embedded is 3.

    Similarly, the distribution pertaining to the MNIST dataset is not truly 784 dimensional despite samples being embedded in $\{0,\dots,255\}^784$. For example, the bottom right pixel in every sample in the training data is black (i.e. pixel value of 0).
\end{example}
Pedro Domingos refers to this observation as the blessing of non-uniformity. It turns out that the space between points on the manifold don't blow up as quickly as one would fear in practice. Tons of regularity to be exploited, i.e. not particularly uniform!
...
\subsection{\TODO Backwards-compatible definition of AI}
Intent is key

\subsection{\TODO The sufficiency of internet-scraped data}

\subsection{\TODO The principled measure-explainability tradeoff}
