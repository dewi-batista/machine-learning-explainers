...
Perhaps the most natural question when shown a new generative probabilistic model class is the following: how might it be applied to (exact and approxiamte) probabilistic inference? As for exact inference, Bayesian networks suffer a major pitfall: the complexity of answering marginal queries is exponential in the tree-width of the Bayesian network. Known algorithms for answering marginal queries, which will be discussed shortly, are variable elimination and junction tree-based algorithms. For example, variable elimination has complexity $O(n\cdot\exp(w))$ where $n$ is the number of nodes and $w$ is the tree-width of the Bayesian network. As such, to understand the difficulty of answering marginal queries in this context, an understanding of the tree-width of a Bayesian network is needed.
